...
class Factory
{
    ...
    public static function getWorldCatUtils(ServiceManager $sm)
    {
     ...
     /**
     * Construct the ArchivesSpace Connector
     * vufind-archivesspace project
     *
     * @param ServiceManager $sm Service manager.
     *
     * @return \VuFind\Connection\ArchivesSpaceConnection
     */
    public static function getArchivesSpaceConnection(ServiceManager $sm) {
      $config = $sm->get('VuFind\Config')->get('archivesspace');
      $client = $sm->get('VuFind\Http')->createClient();
      return new \VuFind\Connection\ArchivesSpaceConnection($client,$config);
    }
}
